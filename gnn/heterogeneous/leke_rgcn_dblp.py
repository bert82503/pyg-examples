...
from gnn.heterogeneous.leke_dblp import LeKeDBLP

path = osp.join(osp.dirname(osp.realpath(__file__)), '../../', 'dataset', 'LeKeDBLP')
print(f"path: {path}")
...
# 数据处理
# 导入数据：
dataset = LeKeDBLP(root=path)
# dataset = LeKeDBLP(root=path, transform=transform)
graph = dataset[0]
...
# 节点分类数量
num_classes = torch.max(graph['user'].y).item() + 1
train_mask, val_mask, test_mask = graph['user'].train_mask, graph['user'].val_mask, graph['user'].test_mask
y = graph['user'].y

node_types, edge_types = graph.metadata()
num_nodes = graph['user'].x.shape[0]
